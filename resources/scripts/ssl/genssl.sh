...
CN = admission-webhook-server.admission-webhook.svc
[ v3_req ]
...
[alt_names]
DNS.1 = admission-webhook-server.admission-webhook.svc
EOF
...
openssl req -new -key k8s-admission-webhook-server.key -subj "/CN=admission-webhook-server.admission-webhook.svc" -config server.conf \
